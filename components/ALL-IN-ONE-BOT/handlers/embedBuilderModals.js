...
class EmbedBuilderModalHandler {
    ...
    async showColorModal(interaction, embed) {
        ...
        const colorInput = new TextInputBuilder()
            .setCustomId('color')
            .setLabel('Hex Color (e.g., #1557ff)')
            .setStyle(TextInputStyle.Short)
            .setValue(embed.color || '#1557ff')
            .setMaxLength(7)
            .setRequired(true)
            .setPlaceholder('#1557ff');
    ...
    async handleColorModal(interaction, embed) {
        ...
        if (!/^#([0-9A-F]{6})$/i.test(color)) {
            return interaction.reply({
                content: '❌ Invalid hex color format. Please use format like #1557ff',
                ephemeral: true
            });
        }
    ...
    getSetProperties(embed) {
        ...
        if (embed.color !== '#1557ff') properties.push('Color');
