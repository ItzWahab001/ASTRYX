  ...
  async execute(interaction) {
    ...
    if (['queue', 'nowplaying'].includes(subcommand)) {
      ...
      if (subcommand === 'queue') {
        if (!queue || !queue.songs.length) {
          const message = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 The queue is empty.')],
          });
        ...
        if (page > totalPages) {
          const message = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription(`🚫 Page ${page} doesn't exist. Total pages: ${totalPages}`)],
            ephemeral: true,
          });
      ...
      if (subcommand === 'nowplaying') {
        if (!queue || !queue.songs.length) {
          const message = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 Nothing is currently playing.')],
          });
    ...
    if (!channel) {
      const message = await interaction.followUp({
        embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 You need to be in a voice channel to use music commands.')],
        ephemeral: true,
      });
      this.autoDeleteMessage(message);
      return;
    ...
    try {
      switch (subcommand) {
        ...
        case 'url':
          if (subcommand === 'play') {
            ...
            if (!searchResult || !searchResult.videos.length) {
              const message = await interaction.followUp({
                embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 No songs found for your query.')],
              });
              this.autoDeleteMessage(message);
              return;
            }
              ...
              try {
                ...
                if (selectedVideo) {
                  ...
                  const errorMessage = await interaction.followUp({
                    embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 The selected song could not be found.')],
                  });
                ...
                const errorMessage = await interaction.followUp({
                  embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 An error occurred while trying to play the song.')],
                });
                this.autoDeleteMessage(errorMessage);
              }
            ...
            collector.on('end', async collected => {
              if (!collected.size) {
                ...
                const timeoutMessage = await interaction.followUp({
                  embeds: [new EmbedBuilder().setColor('#fffffffff').setDescription('⚠️ You didn\'t select any song in time.')],
                });
            ...
            try {
              ...
              const errorMessage = await queueMessage.edit({
                embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 Failed to load song from URL. Please check if the URL is valid.')],
              });
              this.autoDeleteMessage(errorMessage);
            }
          }
        ...
        case 'pause':
          ...
          if (queue.paused) {
            const message = await interaction.followUp({
              embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('⏸️ The song is already paused.')],
            });
            this.autoDeleteMessage(message);
            return;
          }
          ...
          const pauseMessage = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#285cff').setDescription('⏸️ Paused the current song.')],
          });
        ...
        case 'resume':
          ...
          if (!queue.paused) {
            const message = await interaction.followUp({
              embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('▶️ The song is not paused.')],
            });
            this.autoDeleteMessage(message);
            return;
          }
          ...
          const resumeMessage = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#3d7cff').setDescription('▶️ Resumed the current song.')],
          });
        ...
        case 'skip':
          ...
          if (queue.songs.length === 1) {
            const message = await interaction.followUp({
              embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 No more songs in the queue to skip to.')],
            });
            this.autoDeleteMessage(message);
            return;
          }
          ...
          const skipMessage = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#3d7cff').setDescription(`⏭️ Skipped: **${skippedSong.name}**`)],
          });
        ...
        case 'stop':
          ...
          const stopMessage = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#0a46ff').setDescription('⏹️ Stopped playing and cleared the queue.')],
          });
        ...
        case 'loop':
          ...
          const loopMessage = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#3d7cff').setDescription(`🔁 Loop mode set to: **${this.getLoopModeText(mode)}**`)],
          });
        ...
        case 'volume':
          ...
          const volumeMessage = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#3d7cff').setDescription(`🔊 Volume set to **${volume}%**`)],
          });
        ...
        case 'shuffle':
          if (!queue || queue.songs.length <= 2) {
            const message = await interaction.followUp({
              embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 Need at least 2 songs in queue to shuffle.')],
            });
            this.autoDeleteMessage(message);
            return;
          }
          ...
          const shuffleMessage = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#3d7cff').setDescription('🔀 Shuffled the queue!')],
          });
        ...
        case 'remove':
          if (!queue || queue.songs.length <= 1) {
            const message = await interaction.followUp({
              embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 No songs in queue to remove.')],
            });
            this.autoDeleteMessage(message);
            return;
          }
          ...
          if (position === 1) {
            const message = await interaction.followUp({
              embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 Cannot remove the currently playing song. Use skip instead.')],
            });
            this.autoDeleteMessage(message);
            return;
          }
          if (position > queue.songs.length) {
            const message = await interaction.followUp({
              embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription(`🚫 Position ${position} doesn't exist in queue.`)],
            });
            this.autoDeleteMessage(message);
            return;
          }
          ...
          const removeMessage = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#3d7cff').setDescription(`🗑️ Removed: **${removedSong.name}** from position ${position}`)],
          });
        ...
        default:
          const message = await interaction.followUp({
            embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 Unknown command.')],
          });
          this.autoDeleteMessage(message);
          break;
      }
      ...
      const errorMessage = await interaction.followUp({
        embeds: [new EmbedBuilder().setColor('#dbe7ff').setDescription('🚫 An error occurred while processing your request.')],
      });
      this.autoDeleteMessage(errorMessage);
    }
  },
