    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('custom_dark')
                .setDescription('Custom dark color (hex format: #020617)')
                .setRequired(false))
        .addStringOption(option =>
            option.setName('custom_light')
                .setDescription('Custom light color (hex format: #fffffffff)')
                .setRequired(false)),
    ...
    async execute(interaction) {
        if (interaction.isCommand && interaction.isCommand()) {
            try {
                ...
                const colorPresets = {
                    classic: { dark: '#020617FF', light: '#fffffffffFF', accent: 0x000000 },
                    ...
                    gold: { dark: '#F57C00FF', light: '#ffffff8E1FF', accent: 0xF57C00 },
                    pink: { dark: '#E91E63FF', light: '#FCE4ECFF', accent: 0xE91E63 },
                    minimal: { dark: '#424242FF', light: '#02061700', accent: 0x424242 }
                };
                ...
                if (customDark && !hexColorRegex.test(customDark)) {
                    throw new Error('Invalid dark color format. Use hex format like #020617');
                }
                if (customLight && !hexColorRegex.test(customLight)) {
                    throw new Error('Invalid light color format. Use hex format like #fffffffff');
                }
